...
class Processor extends Decoder {
  ...
  * chaoticNeutral(iter) {
  ...
  declareNotation(notation) {
    if (this.notations.has(notation.name)) {
      this._expected(
        undefined,
        `notation ${ notation.name } to be declared only one`
      );
    }

    this.notations.set(notation.name, notation);
  }
  ...
  * ATTLIST_DECL() {
    ...
    cp = yield * this.seriesIs(ATTLIST_CPS, undefined, A_UPPER);
    cp = yield * this.oneOrMoreWhere(isWhitespaceChar);
    cp = yield * this.oneWhere(isNameStartChar, elemNameCPs, cp);
    cp = yield * this.zeroOrMoreWhere(isNameContinueChar, elemNameCPs);
    ...
    while (true) {
      ...
      cp = yield * this.oneWhere(isNameStartChar, attrNameCPs, cp);
      cp = yield * this.zeroOrMoreWhere(isNameContinueChar, attrNameCPs);
      cp = yield * this.oneOrMoreWhere(isWhitespaceChar, undefined, cp);

      const attr = {
        elem: String.fromCodePoint(...elemNameCPs),
        name: String.fromCodePoint(...attrNameCPs)
      };

      // Attlist is the most complex production in XML, with the most branching
      // and the most undelimited syntax (e.g. there are a bunch of cases where
      // ambiguities are only resolved by variable-length sequences).
      ...
      switch (cp) {
        case C_UPPER:
          attr.type = 'STRING';
          cp = yield * this.seriesIs(CDATA_CPS, undefined, cp);
          cp = yield;
          break;

        case E_UPPER:
          attr.type = 'NAME';
          attr.isUnparsedEntityReference = true;

          cp = yield * this.seriesIs([ N_UPPER, T_UPPER, I_UPPER, T_UPPER ]);
          cp = yield;
          ...
          if (cp === Y_UPPER) {
            ...
            attr.plural = true;
            cp = yield * this.seriesIs([
              I_UPPER, E_UPPER, S_UPPER
            ], undefined, cp);
          }
        ...
        case I_UPPER:
          ...
          cp = yield * this.seriesIs(IDREF_CPS, undefined, cp);
          cp = yield;
        ...
        case N_UPPER:
          ...
          if (cp === M_UPPER) {
            ...
            cp = yield * this.seriesIs([
              T_UPPER, O_UPPER, K_UPPER, E_UPPER, N_UPPER
            ]);
            ...
            attr.type = 'NAME';
            attr.isNotation = true;
            attr.enumeration = new Set();

            cp = yield * this.seriesIs([
              T_UPPER, A_UPPER, T_UPPER, I_UPPER, O_UPPER, N_UPPER
            ]);

            cp = yield * this.oneOrMoreWhere(isWhitespaceChar, undefined, cp);
            cp = yield * this.oneIs(PARENTHESIS_LEFT, undefined, cp);
            ...
            while (true) {
              ...
              cp =
                yield * this.oneWhere(isNameStartChar, enumCPs, cp);
              cp =
                yield * this.zeroOrMoreWhere(isNameContinueChar, enumCPs);
              cp =
                yield * this.zeroOrMoreWhere(isWhitespaceChar, undefined, cp);

              const notationName = String.fromCodePoint(...enumCPs);

              if (attr.enumeration.has(notationName)) {
                this._expected(cp, `not to see ${ notationName } twice`);
              }

              attr.enumeration.add(notationName);
        ...
        case PARENTHESIS_LEFT:
          ...
          while (true) {
            ...
            cp = yield * this.oneOrMoreWhere(isNameContinueChar, enumCPs, cp);
            cp = yield * this.zeroOrMoreWhere(isWhitespaceChar, undefined, cp);

            const enumName = String.fromCodePoint(...enumCPs);

            if (attr.enumeration.has(enumName)) {
              this._expected(cp, `not to see token ${ enumName } twice`);
            }

            attr.enumeration.add(enumName);
      ...
      this.oneOrMoreWhere(isWhitespaceChar, undefined, cp);

      const defaultValueCPs = [];
      ...
      if (cp === HASH_SIGN) {
        ...
        switch (cp) {
          case I_UPPER:
            ...
            cp = yield * this.seriesIs([ IMPLIED_CPS ], undefined, cp);
            break;
          case R_UPPER:
            attr.hasDefaultValue = false;
            attr.isRequired      = true;
            cp = yield * this.seriesIs([ REQUIRED_CPS ], undefined, cp);
            break;
          case F_UPPER:
            attr.hasDefaultValue     = true;
            attr.defaultValueIsFixed = true;
            attr.isRequired          = true;
            cp = yield * this.seriesIs([ FIXED_CPS ], undefined, cp);
            cp = yield * this.oneOrMoreWhere(isWhitespaceChar);
            break
        }
      }
  ...
  * DOCUMENT_xmlDecl_standalone() {
    ...
    this.standalone = series === YES_CPS;
  }
  ...
  * NOTATION_DECL() {
    ...
    cp = yield * this.seriesIs(NOTATION_CPS, undefined, N_UPPER);
    cp = yield * this.oneOrMoreWhere(isWhitespaceChar);
    cp = yield * this.oneWhere(isNameStartChar, nameCPs, cp);
    cp = yield * this.zeroOrMoreWhere(isNameContinueChar, nameCPs);

    notation.name = String.fromCodePoint(...nameCPs);

    if (this.notations.has(notation.name)) {
      this._expected(cp, `notation ${ notation.name } to be declared once`);
    }

    cp = yield * this.oneOrMoreWhere(isWhitespaceChar, undefined, cp);

    if (cp !== P_UPPER && cp !== S_UPPER) {
      this._expected(cp, '"PUBLIC" or "SYSTEM"');
    }
    ...
    if (cp === P_UPPER) {
      ...
      notation.publicID = String.fromCodePoint(...publicIDCPs);

      if (cp !== delim) {
        this._expected(cp, 'valid public ID character or delimiter');
      }

      // Unlike other cases, system ID is optional here.

      cp = yield;

      if (cp === GREATER_THAN) {
        this.declareNotation(notation);
        return;
      }

      if (!isWhitespaceChar(cp)) {
        this._expected(cp, 'whitespace or ">"');
      }
      ...
      if (cp !== QUOTE_DBL && cp !== QUOTE_SNG) {
        this._expected(cp, 'quoted system ID value or ">"');
      }
    } else if (cp === S_UPPER) {
      const systemIDCPs = [];
    ...
    notation.systemID = String.fromCodePoint(...systemIDCPs);

    if (cp !== delim) {
      this._expected(cp, 'valid system ID character or delimiter');
    }

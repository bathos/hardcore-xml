...
export class EncodingError extends HardcoreError {
  constructor(realEnc, statedEnc) {
    const msg = statedEnc ?
      `The declared encoding (${ statedEnc }) is not consistent with the ` +
      `actual encoding (${ realEnc } or similar).` :
      `The input appears to be in an unsupported ` +
      `encoding (${ realEnc } or similar).`

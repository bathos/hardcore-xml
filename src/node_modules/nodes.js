...
const coerceString = node => node.text || node.cleanContent || '';

const coerceNumber = node => {
...
const coerceDate = node => {
	const str = coerceString(node);

	return new Date(str);
};

const coerceBoolean = node => {
	const str = coerceString(node);
...
const normRule = ($rule, $nmsm, $renamer) => {
	...
	const { plural, collapse, asArray } = $rule;
...
class Node {
	...
	* lineage() {
	...
	[$toObject](target, rules, plurals, $nmsm, renamer) {
		...
		let self = fold ? {} : rule.before(this, this[$$self]);

		if (rule.asArray && !(self instanceof Array))
			self = [];

		const isOb = self instanceof Object;
		const kids = (isOb || rule.collapse) ? this[$$kids] : [];
		...
		if (plurals.has(prev)) {
			prev.push(res);
		} else if (target[name] && (rule.plural !== false)) {
	...
	toJSON(opts) {
		...
		const tab = 'tab' in opts ? opts.tab : '\t';

		return JSON.stringify(obj, undefined, tab)
	}
...
export class Document extends NodeWithChildren {
	...
	set root(el) {
		...
		if (index)
			this.children[$array][index] = el;
		else
			this.children[$array].push(el);
	}
...
class HTMLElementAttributes extends ElementAttributes {
	...
	setAttribute({ namespace, key, value }) {
		...
		if (existingMember)
			...
			this.push(new HTMLAttribute({ namespace, key, value }));
	}

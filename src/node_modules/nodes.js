...
export class DocumentFragment extends ElementChildren {
	constructor() {
		super();
	}	
}
...
export class Document extends NodeWithChildren {
	constructor({ version, encoding, standalone, children }={}) {
		...
		if (opts.children)
			for (const child in opts.children)
...
class Declaration extends Node {
	constructor(namespace, name) {
		...
		if (!this.valid)
			throw new InvalidDeclarationError();
	}
...
export class AttributeDefinition extends Declaration {
			...
			if (!this.type || this.type == 'NOTATION') {
				...
				if (val.every(m => isValid[thing](m))) {
					...
					this[$members].toString = () => {
						const p = this.type == 'NOTATION' ? 'NOTATION ' : '';
						return `${ p }(${ val.join('|') })`;
					};
				...
				throw new IncompatibleContentSpecError();
			}
		}
	}
	...
	toString({ d=0, pretty=true, quote='"', tab='\t', withNS=true }={}) {
		...
		const type = this.members.toString();
...
export class NotationDeclaration extends Declaration {
	...
	toString({ d=0, quote='"', tab='\t' }={}) {
		if (!this.isValid()) throw new InvalidDeclarationError();

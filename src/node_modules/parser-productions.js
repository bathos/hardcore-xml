...
//
// Non-deterministic patterns are permissable to a certain extent (though are
// best avoided). Specifically, we must at least be able to say that any
// production which *becomes*, *collects*, or *emits* is, when resolved, a
// deterministic match, whereas it doesn’t really matter (except perhaps from a
// performance standpoint) if two choices in a disjunction begin with the same
// token(s) provided neither would resolve a becoming/collecting/emitting
// production by matching that token. That’s a lot of abstract words, but I can
// put it in more practical terms: if you have a disjunction with two paths,
// it’s important that if path A yields a comment, it can’t then fail to resolve
// in favor of path B, since we *will* emit that first comment.
//
...
export const $attTypeEnumeration = {
  pattern: '@LP $attTypeEnum ( $pipe $attTypeEnum )* $rp'
};
...
export const $csMixed = {
  pattern: '@LP #PCDATA ($rp | ($pipe $csMixedName )* $rp * )',
  emits: 'content spec',
  takes: [ 'mixedElems' ]
};
...
export const $elementContent = {
  pattern: '$cdSect | $procInst | $comment | $element | $text'
  expands: '$geRef'
};
...
export const $prolog = {
  pattern: '$prologProlog $misc* ($doctypeDecl $misc*)?'
};

export const $prologProlog = {
  pattern: '$xmlDecl?',
  emits: 'passed xml declaration',
  takes: []
};
